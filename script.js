...
function generateRacePoints(riders, races = 10) {
    return riders.map(rider => {
      let points = Array.from({ length: races }, () => Math.floor(Math.random() * 26));
  ...
  function generateRanking(ridersWithPoints) {
    return ridersWithPoints.sort((a, b) => b.totalPoints - a.totalPoints).map((rider, index) => ({
      position: index + 1,
      name: rider.name,
      team: rider.team,
  ...
  fetch("piloti.json")
    ...
    .then(data => {
      ...
      const ranking = generateRanking(ridersWithPoints);
